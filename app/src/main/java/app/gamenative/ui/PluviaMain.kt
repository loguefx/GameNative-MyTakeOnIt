...
import app.gamenative.launch.LaunchOrchestrator
...
fun preLaunchApp(
    ...
    CoroutineScope(Dispatchers.IO).launch {
        ...
        if (!bootToContainer) {
            ...
            val prefixPath = container.rootDir.absolutePath
            val gameBinaryHostPath = when (gameSource) {
                GameSource.STEAM, GameSource.CUSTOM_GAME, GameSource.GOG -> {
                    val aDrive = ContainerUtils.getADrivePath(container.drives)
                    if (!aDrive.isNullOrEmpty() && effectiveExe.isNotEmpty()) {
                        "$aDrive/${effectiveExe.replace("\\", "/")}"
                    } else null
                }
                GameSource.EPIC -> EpicService.getEpicGameOf(gameId)?.installPath?.let { base ->
                    if (effectiveExe.isNotEmpty()) "$base/${effectiveExe.replace("\\", "/")}" else null
                }
                else -> null
            }
            val preflightResult = LaunchOrchestrator.runPreflight(
                context = context,
                appId = appId,
                gameBinaryPath = gameBinaryHostPath,
                prefixPath = prefixPath,
            )
            if (preflightResult is PreflightResult.Blocked) {
                setLoadingDialogVisible(false)
                setMessageDialogState(
                    MessageDialogState(
                        visible = true,
                        type = DialogType.PREFLIGHT_BLOCKED,
                        title = context.getString(R.string.preflight_blocked_title),
                        message = (preflightResult as PreflightResult.Blocked).reason,
                        dismissBtnText = context.getString(R.string.ok),
                    ),
                )
                return@launch
