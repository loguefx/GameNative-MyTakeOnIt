...
abstract class BaseAppScreen {
    ...
    protected open fun getLastRunDiagnosticsOption(
        context: Context,
        libraryItem: LibraryItem,
        onShowLastRunDiagnostics: (() -> Unit)?,
    ): AppMenuOption? {
        if (onShowLastRunDiagnostics == null) return null
        return AppMenuOption(
            optionType = AppOptionMenuType.LastRunDiagnostics,
            onClick = onShowLastRunDiagnostics,
        )
    }
    ...
    fun Content(
        ...
        if (showDiagnosticsDialog) {
            AlertDialog(
                ...
                title = { Text(context.getString(R.string.diagnostics_last_run_title)) },
                text = {
